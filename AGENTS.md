...
Vendored from [krusemediallc/arcads-claude-code](https://github.com/krusemediallc/arcads-claude-code) (MIT licensed — see `THIRD_PARTY_LICENSES/arcads-claude-code/LICENSE`). `skills/` and `shared/` are the canonical, editable source; `.claude/skills/` and `.cursor/skills/` are generated from them by `scripts/sync-skill.sh` (also runs automatically via the Claude Code `SessionStart` hook in `.claude/settings.json`) and are committed directly rather than gitignored, so the skill works immediately after a fresh clone with no build step required.
